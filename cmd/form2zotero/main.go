...
func main() {
	...
	for key, zoterogroup := range t2zot {
		form.IterateObjects(key, zoterogroup, func(obj *Object) error {
			...
			if err != nil {
				return emperror.Wrapf(err, "cannot load group #%v", zoterogroup)
			}
			item, err := grp.GetItemByOldidLocal(fmt.Sprintf("%v", obj.Objectid))
			if err != nil {
				return emperror.Wrapf(err, "cannot load item by oldid #%v - %v", zoterogroup, obj.Objectid)
			}
			...
			if err != nil {
				return emperror.Wrapf(err, "cannot get rightholders of %v", obj.Objectid)
			}
