...
func (obj *Object) IterateFiles(callback func(file *File) error) error {
	sqlstr := "SELECT masterid, collection, signature, mimetype, `type`, note FROM upload.master WHERE objectid=?"
	obj.form.log.Infof("query: %s", sqlstr)
	rows, err := obj.form.sourceDB.Query(sqlstr, obj.Objectid)
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&f.Masterid, &f.Collection, &f.Signature, &f.Mimetype, &f.Type, &note); err != nil {
